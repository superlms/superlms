{{--
    ...
    Deliberately the same sheet as the academic fee receipt
    (admin/fee-receipt.blade.php) — same header, meta bar, student block,
    payment grid, amount box and footer. The only thing that says this is a
    bus payment is the "Fee Type" badge (Transportation) and the route line.

    The earlier, blue transport-only design is kept at
    admin/transport-receipt-classic.blade.php in case it is wanted back.
--}}
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
    ...
    <div class="sheet">
        ...
        <div class="r-body">
            ...
            <div class="grid">
                <div class="cell"><div class="label">Student Name</div><div class="value">{{ $student->full_name ?? '-' }}</div></div>
                <div class="cell"><div class="label">Admission No.</div><div class="value">{{ $student->admission_no ?? '-' }}</div></div>
                <div class="cell"><div class="label">Class &amp; Section</div><div class="value">{{ $student->standard->name ?? '-' }}{{ $student && $student->section ? ' / ' . $student->section->name : '' }}</div></div>
            ...
            <div class="grid">
                <div class="cell"><div class="label">Fee Type</div><div class="value"><span class="badge badge-transport">Transportation</span></div></div>
                <div class="cell"><div class="label">Payment Mode</div><div class="value">{{ ucfirst(str_replace('_', ' ', (string) $payment->payment_mode)) }}</div></div>
                <div class="cell"><div class="label">Route</div><div class="value">{{ $payment->transportation->route_name ?? '-' }}</div></div>
                <div class="cell"><div class="label">Collected By</div><div class="value">{{ $payment->submittedBy->name ?? '-' }}</div></div>
                @if ($payment->academic_year)
                    <div class="cell"><div class="label">Academic Year</div><div class="value">{{ $payment->academic_year }}</div></div>
                @endif
                @if ($payment->remark)
