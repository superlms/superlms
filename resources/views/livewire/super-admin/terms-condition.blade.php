<div class="min-h-screen bg-gray-50">
    ...
    @if ($activeTab === 'view')
        ...
        @else
            @php
                ...
                $viewAddInfo = $metadata['additional_info'] ?? [];
            ...
            <div class="bg-white border-b border-gray-200 px-4 sm:px-6 py-4 sm:py-5">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    <div class="flex items-center gap-3 min-w-0">
                        ...
                        <div class="min-w-0">
                            <h1 class="text-xl sm:text-2xl font-bold text-gray-900 truncate">{{ $termsCondition->platform_name ?? 'Terms Of Use' }}</h1>
                ...
                @if (count($viewFiles) > 0)
                    <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                        ...
                        <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-3">
                            @foreach ($viewFiles as $index => $file)
                                <div
                                    class="flex items-center justify-between p-4 bg-blue-50 rounded-xl border border-blue-100 gap-3">
                                    <div class="flex items-center gap-3 min-w-0">
                                        <div
                                        ...
                                        <div class="min-w-0">
                                            <p class="text-sm font-semibold text-gray-800 truncate">
                                                {{ $file['title'] ?? 'Document' }}</p>
                                            <p class="text-xs text-gray-400 uppercase">
                                                {{ $file['file_type'] ?? 'PDF' }}</p>
                                        </div>
                                    </div>
                                    <div class="flex items-center gap-1.5 flex-shrink-0">
                                        ...
                                        <button wire:click="confirmDeleteFile({{ $index }})"
    ...
    {{-- ══════════ DELETE ADDITIONAL INFO CONFIRM MODAL ══════════ --}}
    @if ($pendingDeleteAdditionalInfoIndex !== null)
        <div class="fixed inset-0 flex items-center justify-center bg-black/40 backdrop-blur-sm z-[9999] px-4">
            <div class="bg-white rounded-2xl shadow-xl w-full max-w-sm overflow-hidden">
                <div class="px-5 py-4 border-b border-gray-100 bg-red-50 flex items-center gap-3">
                    <div class="w-9 h-9 bg-red-100 rounded-full flex items-center justify-center flex-shrink-0">
                    ...
                    <h3 class="text-sm font-bold text-gray-900">Delete Additional Info</h3>
                </div>
                <div class="p-5">
                    <p class="text-sm text-gray-600">Are you sure you want to remove this additional info entry?
                        This action cannot be undone.
                    </p>
                </div>
                <div class="px-5 pb-5 flex items-center gap-2">
                    <button wire:click="executeRemoveAdditionalInfo"
                        class="flex-1 py-2 bg-red-600 hover:bg-red-700 text-white text-sm font-semibold rounded-lg transition-colors">
                        Yes, Delete
                    </button>
                    <button wire:click="cancelRemoveAdditionalInfo"
                        class="flex-1 py-2 bg-gray-100 hover:bg-gray-200 text-gray-600 text-sm font-medium rounded-lg transition-colors">
                        Cancel
                    </button>
                </div>
    ...
    @if ($pendingDeleteFileIndex !== null)
        <div class="fixed inset-0 flex items-center justify-center bg-black/40 backdrop-blur-sm z-[9999] px-4">
            <div class="bg-white rounded-2xl shadow-xl w-full max-w-sm overflow-hidden">
                <div class="px-5 py-4 border-b border-gray-100 bg-red-50 flex items-center gap-3">
                    <div class="w-9 h-9 bg-red-100 rounded-full flex items-center justify-center flex-shrink-0">
                    ...
                    <h3 class="text-sm font-bold text-gray-900">Delete Document</h3>
                </div>
                <div class="p-5">
                    <p class="text-sm text-gray-600">Are you sure you want to delete this document?
                        It will be permanently removed from storage.
                    </p>
                </div>
                <div class="px-5 pb-5 flex items-center gap-2">
                    <button wire:click="executeDeleteFile"
                        class="flex-1 py-2 bg-red-600 hover:bg-red-700 text-white text-sm font-semibold rounded-lg transition-colors">
                        Yes, Delete
                    </button>
        ...
        <div class="max-w-5xl mx-auto px-6 py-8 space-y-6">
            ...
            <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                ...
                <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" wire:model.defer="platform_name" placeholder="e.g. EduLMS"
                    ...
                    <div>
                        ...
                        <input type="text" wire:model.defer="company_name" placeholder="Legal company name"
                            class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm
                                   focus:ring-2 focus:ring-blue-400 focus:border-blue-400">
                        @error('company_name')
                            <p class="text-xs text-red-500 mt-0.5">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <input type="text" wire:model.defer="company_cin" placeholder="Corporate Identity Number"
                            class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm
                                   focus:ring-2 focus:ring-blue-400 focus:border-blue-400">
                    </div>
                    <div>
                    ...
                    <div class="md:col-span-2">
                        <label class="block text-xs font-medium text-gray-600 mb-1">Platform Logo</label>
                        @if ($logoPreview)
                        ...
                        @endif
                        <input type="file" wire:model="platform_logo" accept="image/*"
            ...
            <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                ...
                <div class="p-6 space-y-3">
                    ...
                    @php $savedFiles = $termsCondition ? ($termsCondition->metadata['files'] ?? []) : []; @endphp
                    @if (count($savedFiles) > 0)
                        <p class="text-xs font-semibold text-gray-500 uppercase tracking-wide">
                            Saved Documents ({{ count($savedFiles) }})
                        </p>
                        @foreach ($savedFiles as $si => $sf)
                            <div class="flex items-center justify-between p-3 bg-blue-50 rounded-xl border border-blue-100 gap-3">
                                <div class="flex items-center gap-3 min-w-0">
                                    ...
                                    <div class="min-w-0">
                                        <p class="text-sm font-semibold text-gray-800 truncate">{{ $sf['title'] ?? 'Document' }}</p>
                                        <p class="text-xs text-gray-400 uppercase">{{ $sf['file_type'] ?? 'file' }}</p>
                                    </div>
                                </div>
                                <div class="flex items-center gap-1.5 flex-shrink-0">
                                    <a href="{{ $sf['file_path'] }}" target="_blank"
                                        class="p-1.5 rounded-lg border border-blue-200 text-blue-600 hover:bg-blue-100 transition-colors" title="View">
                                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" />
                                        </svg>
                                    </a>
                                    <button wire:click="confirmDeleteFile({{ $si }})"
                        ...
                        <div class="border-t border-dashed border-gray-200 pt-3">
                            <p class="text-xs font-semibold text-gray-500 uppercase tracking-wide">Upload New Documents</p>
                        </div>
                    @endif
