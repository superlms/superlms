            ...
            @if (count($additionalInfo) > 0)
                <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                    <div
                        ...
                        <span
                            class="w-8 h-8 bg-gradient-to-br from-indigo-500 to-blue-600
                            text-white rounded-xl flex items-center justify-center flex-shrink-0 shadow-sm">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                            </svg>
                            ...
                            @foreach ($additionalInfo as $info)
                                @if (!empty($info['key']) && !empty($info['value']))
                                    <div
                                        ...
                                        <p class="text-sm text-gray-600 leading-relaxed">{{ $info['value'] }}</p>
                                    </div>
                                @endif
                            @endforeach
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            @foreach ($files as $file)
                                <div
                                    ...
                                    border border-gray-200 hover:border-indigo-200 transition-all duration-200">
                                    <div class="flex items-center gap-3">
                                        <div
                                            class="w-10 h-10 bg-indigo-50 rounded-xl flex items-center justify-center flex-shrink-0">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-xs text-gray-400">
                                                {{ strtoupper($file['file_type'] ?? 'PDF') }} Document</p>
                                        </div>
                                    </div>
                                    <a href="{{ $file['file_path'] ?? '#' }}" target="_blank"
                                        class="inline-flex items-center gap-1.5 text-xs font-medium px-3 py-1.5
