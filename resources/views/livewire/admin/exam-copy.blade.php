<div class="min-h-screen bg-gray-50"
    ...
    <div class="bg-white border-b border-gray-200 sticky top-0 z-30">
        ...
        <div class="border-t border-gray-200 bg-gray-50 px-4 sm:px-6 py-3">
            <div class="flex flex-wrap items-center gap-3">
                ...
                <select wire:model.live="filterExam" class="text-xs bg-white border border-gray-200 rounded-md px-2.5 py-1.5 text-gray-700 min-w-[140px]">
                ...
                <select wire:model.live="filterStudent" @disabled(!$filterSection)
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @forelse ($examCopies as $i => $copy)
                                <tr class="hover:bg-gray-50">
                                    ...
                                    {{-- Uploaded → view / edit / delete. Not uploaded → upload only. --}}
                                    <td class="px-4 py-3">
                                        <div class="flex items-center justify-center gap-1">
                                            ...
                                            @else
                                                <button wire:click="openUploadForRow({{ $copy->id }})" title="Upload PDF"
                                                    class="p-1.5 rounded-md border border-blue-200 text-blue-600 bg-blue-50 hover:bg-blue-100">
                                                ...
                                                <td class="px-3 py-2">
                                                    @if ($sp['has_pdf'])
                                                        <div class="flex items-center gap-1">
                                                            ...
                                                            <button wire:click="askDeletePdfInUpload({{ $sid }})" type="button" title="Delete PDF"
    ...
    @if ($showEditModal && !empty($editCopyMeta))
        <div class="fixed inset-0 z-50 overflow-hidden">
            ...
            <div class="absolute top-0 right-0 bottom-0 w-full max-w-xl bg-white shadow-2xl flex flex-col">
                ...
                <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200 flex-shrink-0">
                    <div>
                        <h2 class="text-lg font-semibold text-gray-900">{{ $editMode === 'upload' ? 'Upload Exam Copy' : 'Edit Exam Copy' }}</h2>
                ...
                <div class="flex-1 overflow-y-auto px-6 py-6 space-y-5">
                    ...
                    <div>
                        @if ($editMode === 'upload')
                            <label class="block text-sm font-medium text-gray-700 mb-1.5">Exam Copy PDF <span class="text-red-500">*</span> <span class="text-gray-400 font-normal">(max 5 MB)</span></label>
                        @else
                ...
                <div class="px-6 py-3.5 border-t border-gray-200 flex items-center justify-end gap-2 flex-shrink-0">
                    ...
                    <button wire:click="saveEdit" wire:loading.attr="disabled"
                        class="px-5 py-2 bg-gray-900 hover:bg-gray-800 text-white text-sm font-medium rounded-md disabled:opacity-60">
                        <span wire:loading.remove wire:target="saveEdit">{{ $editMode === 'upload' ? 'Upload' : 'Update' }}</span>
    ...
    {{-- ══════════════════════════════════════════════════
         DELETE CONFIRM — uploaded PDF inside the upload panel
        ...
        <div class="fixed inset-0 z-[60] flex items-center justify-center p-4">
            ...
            <div class="relative bg-white rounded-xl shadow-2xl w-full max-w-sm p-6">
                <div class="flex items-start gap-4">
                    ...
                    <div class="flex-1">
                        <h3 class="text-base font-semibold text-gray-900 mb-1">Delete uploaded PDF?</h3>
                        <p class="text-sm text-gray-500">
                            {{ $studentPdfs[$pendingUploadDeleteStudent]['student_name'] ?? 'This student' }}'s uploaded copy will be permanently removed. The record stays and can be uploaded again.
                        </p>
                    </div>
                </div>
                <div class="flex items-center justify-end gap-2 mt-5">
                    <button wire:click="cancelUploadDelete" class="px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-100 rounded-md">Cancel</button>
                    <button wire:click="confirmDeletePdfInUpload" wire:loading.attr="disabled"
                        class="px-4 py-2 text-sm font-medium text-white bg-red-600 hover:bg-red-700 rounded-md disabled:opacity-60">
                        <span wire:loading.remove wire:target="confirmDeletePdfInUpload">Delete</span>
                        <span wire:loading wire:target="confirmDeletePdfInUpload">Deleting...</span>
                    </button>
                </div>
            </div>
        </div>
