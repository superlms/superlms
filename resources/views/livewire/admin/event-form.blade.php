...
<div>
    <form wire:submit.prevent="save" class="space-y-4">
        ...
        {{-- Color — fixed 12-swatch palette, pick one by clicking. The Alpine
             entangle keeps the picked value in sync without a round trip. --}}
        <div x-data="{ picked: @entangle('color') }">
            <div class="flex items-center justify-between mb-1.5">
                <label class="block text-sm font-medium text-gray-700">Color</label>
                <span class="flex items-center gap-1.5 text-xs text-gray-400">
                    <span class="w-3.5 h-3.5 rounded-full border border-black/10"
                        :style="'background-color: ' + (picked || '')"></span>
                    <span x-text="picked"></span>
                </span>
            </div>
            <div class="flex flex-wrap gap-2.5 px-3.5 py-3 border border-gray-300 rounded-md">
                @foreach ($colorOptions as $hex)
                    <button type="button" title="{{ $hex }}"
                        ...
                        :class="(picked || '').toLowerCase() === @js(strtolower($hex))
                            ...
                            : 'hover:scale-110'"
                        class="w-8 h-8 rounded-full border border-black/10 shadow-sm transition-transform focus:outline-none"
                        ...
                        <svg x-show="(picked || '').toLowerCase() === @js(strtolower($hex))" style="display: none"
                            class="w-4 h-4 mx-auto text-white" fill="none" stroke="currentColor" stroke-width="3"
                            viewBox="0 0 24 24">
