<div class="min-h-screen bg-gray-50">
    ...
    <div class="bg-white border-b border-gray-200 sticky top-0 z-30">
        ...
        {{-- ══════════ FILTER BAND — attached, full width ══════════ --}}
        @if ($activeTab === 'academic')
            <div class="border-t border-gray-200 bg-gray-50 px-4 sm:px-6 py-3">
                <div class="flex flex-wrap items-center gap-3">
                    ...
                    <select wire:model.live="filterStructureStandard" class="text-xs bg-white border border-gray-200 rounded-md px-3 py-1.5 text-gray-700 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500">
                        <option value="">Select Class</option>
                        @foreach ($standards as $std)<option value="{{ $std->id }}">{{ $std->name }}</option>@endforeach
                    </select>
                    <select wire:model.live="filterStructureSection" @disabled(!$filterStructureStandard) class="text-xs bg-white border border-gray-200 rounded-md px-3 py-1.5 text-gray-700 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 disabled:opacity-50">
                    ...
                    <input wire:model.live.debounce.300ms="search" type="text" placeholder="Search fee name…"
                        class="text-xs bg-white border border-gray-200 rounded-md px-3 py-1.5 text-gray-700 w-44 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500">
                    @if ($filterStructureStandard || $filterStructureSection || $search)
                        <button wire:click="clearStructureFilters" class="text-xs text-emerald-600 hover:text-emerald-800 font-medium">Clear</button>
                    @endif
                </div>
            </div>
            ...
            <div class="border-t border-gray-200 bg-gray-50 px-4 sm:px-6 py-3">
                <div class="flex flex-wrap items-center gap-3">
                    ...
                    <input wire:model.live.debounce.300ms="search" type="text" placeholder="Search route…"
                        class="text-xs bg-white border border-gray-200 rounded-md px-3 py-1.5 text-gray-700 w-56 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500">
    ...
    <div class="p-4 sm:p-6 space-y-5">
        ...
        @if ($activeTab === 'academic')
            @if (!$filterStructureStandard)
                {{-- Filter-gated: nothing shows until a class is chosen --}}
                <div class="bg-white rounded-2xl border border-dashed border-gray-200 px-4 py-16 text-center">
                    <div class="w-14 h-14 mx-auto mb-4 rounded-2xl bg-emerald-50 flex items-center justify-center">
                        <svg class="w-7 h-7 text-emerald-400" fill="none" stroke="currentColor" stroke-width="1.6" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z" /></svg>
                    </div>
                    <p class="text-sm font-semibold text-gray-800">Select a class to view its fee structure</p>
                    <p class="text-xs text-gray-400 mt-1">Pick a class (and optionally a section) from the filter above.</p>
                </div>
            @else
                {{-- Results toolbar: count + icon-only Download --}}
                <div class="flex items-center justify-between gap-3">
                    <p class="text-sm text-gray-600">
                        <strong class="text-gray-900">{{ $structureGroups->count() }}</strong>
                        fee structure{{ $structureGroups->count() === 1 ? '' : 's' }}
                    </p>
                    <div class="flex items-center gap-1.5">
                        <a href="{{ route('accounts.fee-structure.pdf', ['organization' => auth()->user()->organization_id, 'standard' => $filterStructureStandard, 'section' => $filterStructureSection, 'download' => 1]) }}" target="_blank" title="Download all"
                            class="w-9 h-9 inline-flex items-center justify-center rounded-lg border border-gray-200 bg-white text-gray-500 hover:bg-gray-50 hover:text-gray-700 transition-colors">
                            <svg class="w-[18px] h-[18px]" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                        </a>
                    </div>
                </div>

                {{-- Clean, minimalist cards: one per class + section --}}
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    @forelse ($structureGroups as $g)
                        <div wire:key="grp-{{ $g['standard_id'] }}-{{ $g['section_id'] ?? 0 }}" class="bg-white rounded-2xl border border-gray-200 hover:shadow-md transition-shadow flex flex-col overflow-hidden">
                            {{-- Card header --}}
                            <div class="px-4 py-3 border-b border-gray-100 flex items-start justify-between gap-2">
                                <div class="min-w-0">
                                    <h3 class="text-sm font-bold text-gray-900 truncate">{{ $g['class'] }} <span class="text-gray-400 font-medium">· {{ $g['section'] }}</span></h3>
                                    <p class="text-[11px] text-gray-400 mt-0.5">{{ $g['year'] }} · {{ count($g['rows']) }} fee head(s)</p>
                                </div>
                                <div class="flex items-center gap-0.5 flex-shrink-0">
                                    <a href="{{ route('accounts.fee-structure.pdf', ['organization' => auth()->user()->organization_id, 'standard' => $g['standard_id'], 'section' => $g['section_id'] ?? '']) }}" target="_blank" title="Print" class="p-1.5 rounded-md text-gray-400 hover:text-gray-700 hover:bg-gray-100">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z"/></svg>
                                    </a>
                                    <button wire:click="viewGroup({{ $g['standard_id'] }}, {{ $g['section_id'] ?? 'null' }})" title="View" class="p-1.5 rounded-md text-gray-400 hover:text-emerald-600 hover:bg-emerald-50">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                    </button>
                                    <button wire:click="editGroup({{ $g['standard_id'] }}, {{ $g['section_id'] ?? 'null' }})" title="Edit" class="p-1.5 rounded-md text-gray-400 hover:text-amber-600 hover:bg-amber-50">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                    </button>
                                    <button wire:click="deleteGroup({{ $g['standard_id'] }}, {{ $g['section_id'] ?? 'null' }})" title="Delete" class="p-1.5 rounded-md text-gray-400 hover:text-red-600 hover:bg-red-50">
                            ...
                            <div class="flex-1 divide-y divide-gray-50">
                                @foreach ($g['rows'] as $r)
                                    <div class="flex items-center justify-between px-4 py-2 text-sm">
                                        <span class="text-gray-600 truncate pr-3">{{ $r->fee_name }}</span>
                                        <span class="font-medium text-gray-800 flex-shrink-0">₹{{ number_format($r->amount, 2) }}</span>
                                    </div>
                                @endforeach
                            </div>
                            {{-- Total --}}
                            <div class="flex items-center justify-between px-4 py-3 bg-gray-50 border-t border-gray-100">
                                ...
                                <span class="text-base font-bold text-emerald-700">₹{{ number_format($g['total'], 2) }}</span>
                            </div>
                        </div>
                    @empty
                        <div class="col-span-full bg-white rounded-2xl border border-dashed border-gray-200 px-4 py-12 text-center">
                            <p class="text-sm font-semibold text-gray-700">No fee structure for this selection</p>
                            <p class="text-xs text-gray-400 mt-1">Use <strong>Add Fee Structure</strong> to create one.</p>
                        </div>
                    @endforelse
                </div>
            @endif
                ...
                @forelse ($routes as $r)
                    <div wire:key="route-{{ $r->id }}" class="bg-white rounded-xl border border-gray-200 p-5">
                        ...
                        <div class="grid grid-cols-2 gap-2 mt-4 text-center">
                            <div class="bg-teal-50 rounded-lg py-2.5">
                                <p class="text-base font-bold text-teal-700">₹{{ number_format($r->monthly_fee, 0) }}</p>
                                <p class="text-[10px] uppercase tracking-wide text-teal-400">Monthly</p>
                            </div>
        ...
        <div class="fixed inset-0 z-50 overflow-hidden">
            ...
            <div class="absolute top-0 right-0 bottom-0 w-full max-w-xl bg-white shadow-2xl flex flex-col">
                ...
                <div class="flex-1 overflow-y-auto px-6 py-6 space-y-5">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <select wire:model.live="structureStandardId" class="w-full border border-gray-300 rounded-md px-3.5 py-2.5 text-sm bg-white focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500">
                                <option value="">Select class…</option>
                                @foreach ($standards as $std)<option value="{{ $std->id }}">{{ $std->name }}</option>@endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input type="text" wire:model="academicYear" placeholder="2026-27" class="w-full border border-gray-300 rounded-md px-3.5 py-2.5 text-sm focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500">
                            ...
                            <div class="border border-gray-200 rounded-md p-3 max-h-32 overflow-y-auto space-y-1.5">
                                @forelse ($formSections as $sec)
                                    <label class="flex items-center gap-2 text-sm text-gray-700">
                                        <input type="checkbox" value="{{ $sec->id }}" wire:model="structureSectionIds" class="rounded border-gray-300 text-emerald-600 focus:ring-emerald-500">
                                        {{ $sec->name }}
                                    </label>
                                @empty
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-2">
                            ...
                            <button wire:click="addFeeRow" class="text-xs font-medium text-emerald-600 hover:text-emerald-800">+ Add row</button>
                        </div>
                        <div class="space-y-2">
                            @foreach ($feeRows as $i => $row)
                                <div wire:key="row-{{ $i }}" class="flex items-center gap-2">
                                    <input type="text" wire:model="feeRows.{{ $i }}.name" placeholder="Fee name (e.g. Tuition)"
                                        class="flex-1 border border-gray-300 rounded-md px-3 py-2 text-sm focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500">
                                    <input type="number" min="0" step="0.01" wire:model="feeRows.{{ $i }}.amount" placeholder="Amount"
                                        class="w-32 border border-gray-300 rounded-md px-3 py-2 text-sm focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500">
                ...
                <div class="px-6 py-3.5 border-t border-gray-200 flex items-center justify-end gap-2 flex-shrink-0">
                    ...
                    <button wire:click="saveStructure" class="px-5 py-2 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-medium rounded-md">{{ $editStructureId || $editingGroup ? 'Update' : 'Add' }}</button>
                </div>
            </div>
        </div>
    @endif
        ...
        <div class="fixed inset-0 z-50 overflow-hidden">
            ...
            <div class="absolute top-0 right-0 bottom-0 w-full max-w-xl bg-white shadow-2xl flex flex-col">
                ...
                <div class="px-6 py-3.5 border-t border-gray-200 flex items-center justify-end flex-shrink-0">
                    <button wire:click="closeViewModal" class="px-5 py-2 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-medium rounded-md">Close</button>
                </div>
            </div>
        </div>
    @endif
                ...
                <div class="flex-1 overflow-y-auto px-6 py-6">
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            ...
                            @endforeach
                            <tr class="border-t-2 border-gray-200"><td colspan="2" class="py-2.5 text-right font-bold text-gray-700 uppercase text-xs">Total</td><td class="py-2.5 text-right font-bold text-emerald-700">₹{{ number_format($viewGroupData['total'], 2) }}</td></tr>
                        </tbody>
                    </table>
                </div>
                <div class="px-6 py-3.5 border-t border-gray-200 flex items-center justify-end flex-shrink-0">
                    <button wire:click="closeViewGroup" class="px-5 py-2 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-medium rounded-md">Close</button>
                </div>
            </div>
        </div>
    @endif
