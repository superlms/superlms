{{--
    Transport fee receipt as a PDF (dompdf) — the student app's receipt and the
    admin download. Plain and quiet: the school at the head, the student and
    the payment as label/value lines, the amount the one loud thing on the
    sheet. Laid out in tables because dompdf has no flexbox.
--}}
<!DOCTYPE html>
<html lang="en">
<head>
...
@php
    ...
    $paymentRows = array_filter([
        ...
        'Received by' => $submittedBy,
        'Session'     => $payment->academic_year,
        'Remark'      => $payment->remark,
    ], fn ($v) => $v !== null && $v !== '' && $v !== '—');

    $routeRows = $route ? array_filter([
        'Route'        => $route->route_name,
        'Per month'    => $money($route->monthly_fee),
        'Months billed'=> $billedMonths ? $billedMonths . ' months' : null,
        'Year total'   => $money($yearTotal),
...
<table class="head">
    <tr>
        ...
        <td class="right">
            <div class="kicker">Transport Fee Receipt</div>
...
@if ($routeRows)
    ...
    <table class="kv">
        @foreach ($routeRows as $label => $value)
            <tr class="{{ $loop->last ? 'last' : '' }}">
                <td class="k">{{ $label }}</td>
                <td class="v">{{ $value }}</td>
            </tr>
...
@else
    ...
    <table class="kv">
        <tr class="last">
            <td class="k">Route</td>
            <td class="v">Not on a route right now</td>
        </tr>
    </table>
@endif
